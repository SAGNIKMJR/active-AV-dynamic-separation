...
class VectorEnvCustom:
    ...
    def __init__(
        ...
        """our task doesn't have num-episodes attribute, so this needs to be commented out"""
        # for write_fn in self._connection_write_fns:
...
class VectorEnvCustom:
    ...
    def __init__(
        ...
        for write_fn in self._connection_write_fns:
            write_fn((OBSERVATION_SPACE_NAME, None))
        self.observation_spaces = [
            read_fn() for read_fn in self._connection_read_fns
        ]
        for write_fn in self._connection_write_fns:
            write_fn((ACTION_SPACE_NAME, None))
        self.action_spaces = [
            read_fn() for read_fn in self._connection_read_fns
        ]
        self._paused = []
        ...
        try:
            ...
            while command != CLOSE_COMMAND:
                if command == STEP_COMMAND:
                    ...
                    command == OBSERVATION_SPACE_NAME
                    or command == ACTION_SPACE_NAME
                ):
                ...
                elif command == CURRENT_EPISODE_NAME:
    ...
    def current_episodes(self):
        self._is_waiting = True
        for write_fn in self._connection_write_fns:
            write_fn((CURRENT_EPISODE_NAME, None))
        results = []
        for read_fn in self._connection_read_fns:
            results.append(read_fn())
        self._is_waiting = False
